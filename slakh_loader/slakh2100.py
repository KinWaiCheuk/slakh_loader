...
class SlakhCollatorv2:
    ...
    def _sample_full(self, batch):
        ...
        target_dict = {}        
        plugin_ids = torch.where(torch.from_numpy(batch[0]['instruments']==1))[0]
        for roll_type, roll in batch[0]['target_dict'][self.ix_to_name[int(plugin_ids[0])]].items():
            target_dict[roll_type] = torch.empty((batch_size, self.plugin_labels_num, *roll.shape))
        unwrapped_batch['waveforms'] = torch.empty(batch_size, batch[0]['waveform'].shape[-1])
        unwrapped_batch['instruments'] = torch.empty(batch_size, batch[0]['instruments'].shape[-1])
        for n in range(batch_size): # Looping through the batchsize
            plugin_ids = torch.where(torch.from_numpy(batch[n]['instruments']==1))[0]
            # Placeholders for a new batch
            # TODO: replace this hard encoded T=1001 with something better
            unwrapped_batch['waveforms'][n] = batch[n]['waveform']
            unwrapped_batch['instruments'][n] = torch.from_numpy(batch[n]['instruments'])
            # creating placeholders for target_dict
            for plugin_id in plugin_ids:
                for roll_type, roll in batch[n]['target_dict'][self.ix_to_name[int(plugin_id)]].items():
